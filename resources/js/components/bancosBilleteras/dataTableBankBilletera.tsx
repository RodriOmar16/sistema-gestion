...
import { BancoBilletera, Paginacion } from "@/types/typeCrud"
...
export function getColumns(
  confirmar: (data: BancoBilletera) => void, 
  openEdit: (data: BancoBilletera) => void): ColumnDef<BancoBilletera>[] {
  return [
    {
      accessorKey: "banco_billetera_id",
      header: ({column}) => {
        return (
          <div className="flex">
            ID
      ...
      cell: ({ row }) => (
        <div className="text-right">{row.getValue("banco_billetera_id")}</div>
      ),
    },
    {
      accessorKey: "nombre",
...
export default function DataTableBancosBilleteras({ datos, openEdit, abrirConfirmar }: Props) {
  ...
  const data = useMemo(() => {
    ...
    return busqueda
      ? datos.filter((campo) =>
          campo.banco_billetera_id?.toString().includes(texto) ||
  ...
  const confirmar = (data: BancoBilletera) => abrirConfirmar(data);
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                ...
                >
                  No hay resultados para mostrar. Utiliza los filtros para obtener bancos o billeteras.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
