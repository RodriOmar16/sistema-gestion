...
export function FiltrosForm({ data, set }:propsForm){
    ...
    <div>
      ...
      <form className='grid grid-cols-12 gap-4 px-4 pt-1 pb-4' onSubmit={handleSubmit}>
        <div className='col-span-12 sm:col-span-6 md:col-span-6 lg:col-span-2'>
          ...
          <Input className='text-right' value={data.caja_id} onChange={(e)=>set({...data, caja_id: Number(e.target.value)}) }/>	
        </div>
...
export default function Cajas(){
  ...
  //effect
  useEffect(() => {
    if (cajas && cajas.data.length > 0) {
      setCacheados(cajas.data);
    } else {
      setCacheados([]);
    }
  }, [cajas]);
      ...
      <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        ...
        <div className="p-4 relative flex-1 overflow-auto rounded-xl border border-sidebar-border/70 md:min-h-min dark:border-sidebar-border">
          <DataTableCajas
            ...
            exportar={data}
            totalFilas={cajas?.total ?? 0}
            current_page={cajas?.current_page ?? 0}
            last_page={cajas?.last_page ?? 0}
            next_page_url={cajas?.next_page_url ?? ''}
            prev_page_url={cajas?.prev_page_url ?? ''}
          />
        </div>
      </div>
      <ModalConfirmar
